...
fun Activity.promptMiuiAutostart(onSkip: () -> Unit) {
    AlertDialog.Builder(this)
        ...
        .setMessage(
            "On Xiaomi devices, autostart must be enabled for Voice to receive incoming calls when the app is closed."
        )
...
fun Activity.promptMiuiAppPermissions(onSkip: () -> Unit) {
    val denied = buildList {
        ...
        if (!isMiuiPermissionGranted(OP_BACKGROUND_START_ACTIVITY))
            add("Display pop-up windows while running in the background")
    ...
    AlertDialog.Builder(this)
        ...
        .setMessage(
            "Voice needs the following permissions on Xiaomi devices to show incoming calls:\n\n$permissionList"
        )
...
private fun Activity.miuiPermissionManagerIntent() =
...
private fun miuiAutostartIntent() =
    Intent().apply {
        component =
